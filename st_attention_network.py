...
class AttentionModel(nn.Module):
    ...
    def forward(self, input_seq, input_seq_length):
        ...
        batch_final_traj_embeddings = torch.transpose(output_refined, 0, 1)[input_seq_length - 1,
                                      torch.arange(input_seq.shape[0]), :]
    ...
    for epoch in range(epochs):
        ...
        if training_loss < train_loss_min:
            ...
            torch.save(model.state_dict(),
                       "model_files/LSTM_net_1" + data_generator.embedding_name)
        print(
...
def get_lstm_model(load_from_file=True):
    if not load_from_file:
        attn_model = AttentionModel(vocab_size=len(data_generator.vocab_to_int) - 3,
                                    location_embedding_size=model_location_embedding_size,
                                    lstm_size=model_lstm_size).to(device)

        optimizer = optim.Adam(attn_model.parameters(), lr=0.0002)
        train(attn_model, optimizer, loss_fn, epochs=100)

    lstm_model = AttentionModel(vocab_size=len(data_generator.vocab_to_int) - 3,
                                location_embedding_size=model_location_embedding_size, lstm_size=model_lstm_size).to(
        device)
    fwd_model_state_dict = torch.load("model_files/LSTM_net_1" + data_generator.embedding_name)
    lstm_model.load_state_dict(fwd_model_state_dict)

    return lstm_model


#print_all(get_lstm_model(False))
